...
export class InterfaceComponent implements OnInit {
  ...
  constructor(
    ...
    this.translate.use(
      browserLang.match(/English|Français|Português/) ? browserLang : 'English'
    );
  }
  ...
  doSomething($event) {
    ...
    if (
      this.child.varChange === true ||
      this.childGroups.groupChange === true
    ) {
      $event.returnValue =
  ...
  ngOnInit() {
    ...
    if (this.dvLocale != null) {
      if (this.dvLocale === 'en') {
        ...
        const browserLang = this.translate.getBrowserLang();
        this.translate.use(
          browserLang.match(/English|Français|Português/)
            ? browserLang
            : 'English'
        );
      }
    ...
    if (!this.siteUrl && this._id != null) {
      ...
      if (this.siteUrl) {
        uri =
          this.siteUrl + '/api/access/datafile/' + this._id + '/metadata/ddi';
        if (this.metaId != null) {
          uri = uri + '?fileMetadataId=' + this.metaId;
        }
      } else {
  ...
  showVars() {
    ...
    for (let i = 0; i < variables.length; i++) {
      ...
      if (typeof obj.var.catgry !== 'undefined') {
        ...
        for (let k = 0; k < obj.var.catgry.length; k++) {
          if (typeof obj.var.catgry[k].catStat !== 'undefined') {
            ...
            sumCount =
              sumCount + parseInt(obj.var.catgry[k].catStat[0]['#text']);
          }
        }
        for (let k = 0; k < obj.var.catgry.length; k++) {
          if (typeof obj.var.catgry[k].catStat !== 'undefined') {
            // tslint:disable-next-line:radix
            obj.var.catgry[k].countPerc =
              (parseInt(obj.var.catgry[k].catStat[0]['#text']) * 100) /
              sumCount;
          }
        }
      ...
      if (typeof obj.var.notes !== 'undefined') {
        if (
          typeof obj.var.notes.length !== undefined &&
          obj.var.notes.length === 2
        ) {
          obj.var.notes = {
  ...
  makeXML(dv) {
    ...
    for (let i = 0; i < this._variables.length; i++) {
      ...
      if (
        typeof this._variables[i]['@wgt'] !== 'undefined' &&
        this._variables[i]['@wgt'] !== ''
      ) {
        doc.writeAttribute('wgt', this._variables[i]['@wgt']);
      }
      if (
        typeof this._variables[i]['@wgt-var'] !== 'undefined' &&
      ...
      if (typeof this._variables[i].location !== 'undefined') {
        doc
          ...
          .writeAttribute('fileid', this._variables[i].location['@fileid']);
        doc.endElement();
      }
      // end location
      // start labl
        ...
        if (typeof this._variables[i].catgry.length !== 'undefined') {
          for (let j = 0; j < this._variables[i].catgry.length; j++) {
            ...
            if (typeof this._variables[i].catgry[j].catValu !== 'undefined') {
              doc
                .startElement('catValu')
                .text(this._variables[i].catgry[j].catValu);
              doc.endElement();
            }
            if (typeof this._variables[i].catgry[j].labl !== 'undefined') {
              ...
              doc.writeAttribute(
                'level',
                this._variables[i].catgry[j].labl['@level']
              );
            ...
            if (typeof this._variables[i].catgry[j].catStat !== 'undefined') {
              // frequency
              if (
                typeof this._variables[i].catgry[j].catStat.length !==
                'undefined'
              ) {
                doc.startElement('catStat');
                doc.writeAttribute(
                  'type',
                  this._variables[i].catgry[j].catStat[0]['@type']
                );
                ...
                if (this._variables[i].catgry[j].catStat.length > 1) {
                  ...
                  doc.writeAttribute(
                    'wgtd',
                    this._variables[i].catgry[j].catStat[1]['@wgtd']
                  );
                  doc.writeAttribute(
                    'type',
                    this._variables[i].catgry[j].catStat[1]['@type']
                  );
      ...
      if (
        ...
        ((typeof this._variables[i].qstn.qstnLit !== 'undefined' &&
          this._variables[i].qstn.qstnLit !== '') ||
          (typeof this._variables[i].qstn.ivuInstr !== 'undefined' &&
            this._variables[i].qstn.ivuInstr !== '') ||
          (typeof this._variables[i].qstn.postQTxt !== 'undefined' &&
            this._variables[i].qstn.postQTxt !== ''))
      ) {
      ...
      if (typeof this._variables[i].notes !== 'undefined') {
        // start notes cdata
        if (
          typeof this._variables[i].notes['#cdata'] !== 'undefined' &&
          this._variables[i].notes['#cdata'] !== ''
        ) {
      ...
      // start universe
      if (
        typeof this._variables[i].universe !== 'undefined' &&
        this._variables[i].universe['#text'] !== ''
      ) {
  ...
  sendToDV() {
    ...
    if (key !== null) {
      ...
      this.ddiService.putDDI(url, doc.toString(), key).subscribe(
        data => {
          console.log('Data ');
          console.log(data);
        },
        error => {
          this.snackBar.open(
            this.translate.instant('SAVE.CANNOT') + error,
            '',
            {
              duration: 2000
            }
